-- 새로운 경제 밸런스 패치: 마켓 정산 시 순수익의 5% 수수료를 차감하는 함수
...
BEGIN
  ...
  LOOP
    IF v_bet.side = p_winning_side THEN
      ...
      -- [NEW] 5% 수수료 차감 (원금을 제외한 순수익 기준)
      v_profit := v_reward - v_bet.amount;
      IF v_profit > 0 THEN
        v_fee := ROUND(v_profit * 0.05);
        v_reward := v_reward - v_fee;
      END IF;
      ...
      VALUES (v_bet.user_id, v_reward, 'reward', '마켓 적중 배당금 (승리: ' || UPPER(p_winning_side) || ', 수수료 5% 제외)');
    END IF;
  END LOOP;

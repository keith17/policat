...
## [0.9.5] - 2026-05-26
### 추가
- 이벤트 및 마켓 URL을 UUID에서 읽기 쉬운 형태(Slug)로 변경 (`/market/category-123456`)
- `update_slugs.sql` 추가 (기존 데이터에 Slug 부여 및 신규 데이터 자동 생성 트리거)
- 동적 Open Graph(OG) 이미지 생성 API 신설 (`/api/og?type=...&slug=...`): 카카오톡, 슬랙 공유 시 실시간 확률 바 렌더링
- 메타데이터에 총 참여 포인트 반영 등 공유 텍스트 개선
- 기존 공유된 UUID 링크 접속 시 자동으로 호환되도록 Fallback 라우팅 적용

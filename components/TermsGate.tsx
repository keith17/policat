...
3. 최종 접속일로부터 180일간 로그인 기록이 없는 경우 휴면 계정으로 전환되며, 보유 중인 모든 포인트는 자동 소멸됩니다.

제5조 (포인트 사용 및 보상 지급)
1. 회원은 누적된 포인트를 사용하여 상점에서 상품을 구매할 수 있습니다.
2. 상품권은 본인인증된 휴대폰 번호로 발송되며, 번호 오기재로 인한 책임은 회원에게 있습니다.
...
본 동의는 선택 사항이며, 거부하시더라도 기본 예측 서비스와 포인트 적립은 정상적으로 이용 가능합니다. 언제든지 설정 메뉴에서 철회할 수 있습니다.`;
...
export default function TermsGate() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      if (!user) return;
      const { data: profile } = await supabase.from("profiles").select("terms_agreed").eq("id", user.id).single();
  ...
  const handleSubmit = async () => {
    if (!userId || !allRequired || saving) return;
    setSaving(true);
    const supabase = createClient();
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(10,10,15,0.85)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      >
        ...
        <div style={{ padding: "16px 28px", borderBottom: "1px solid var(--border)" }}>
          ...
          >
            <div style={{
              width: 22, height: 22, borderRadius: 6, border: `2px solid ${allChecked ? "var(--accent)" : "var(--border)"}`,
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 6, marginLeft: 32 }}>
            필수 및 선택 약관 전체 동의
          </div>
        </div>
            ...
            return (
              <div key={item.key} style={{ borderBottom: "1px solid var(--border)" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 28px" }}>
                  ...
                  >
                    <div style={{
                      width: 20, height: 20, borderRadius: 5, border: `2px solid ${isChecked ? "var(--accent)" : "var(--border)"}`,
                  ...
                  <button
                    ...
                    style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", display: "flex", alignItems: "center", padding: 4, flexShrink: 0 }}
                  >
                  ...
                  {isExpanded && (
                    ...
                    >
                      <div style={{ margin: "0 28px 16px", background: "var(--bg-secondary)", borderRadius: 10, padding: "14px 16px", maxHeight: 200, overflowY: "auto" }}>
                        <pre style={{ margin: 0, fontSize: 12, lineHeight: 1.7, color: "var(--text-secondary)", whiteSpace: "pre-wrap", fontFamily: "inherit" }}>
                          {item.content}
                        </pre>
                        {item.link && (
                          <a href={item.link} target="_blank" rel="noopener noreferrer" style={{ display: "inline-block", marginTop: 10, fontSize: 12, color: "var(--accent)", textDecoration: "underline" }}>
                            전문 보기 →
                          </a>
                        )}
                      </div>
        ...
        <div style={{ padding: "20px 28px", borderTop: "1px solid var(--border)" }}>
          ...
          <motion.button
            ...
            style={{
              ...
              background: allRequired ? "var(--accent)" : "var(--surface-alt)",
              color: allRequired ? "white" : "var(--text-muted)",
              fontSize: 16, fontWeight: 800, cursor: allRequired ? "pointer" : "not-allowed",
              transition: "all 0.2s"
            }}
          >
